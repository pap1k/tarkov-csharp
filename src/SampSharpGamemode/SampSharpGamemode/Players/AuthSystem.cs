                ...
                if (e.DialogButton == DialogButton.Left)
                {
                    if (IsPasswordCorrect(e.InputText))
                    {
                        ...
                        if (dbresult.data.Count > 0)
                        {
                            ...
                            player.SendClientMessage(Colors.SUCCESS, $"Вы успешно авторизовались. Желаем приятной игры на нашем сервере!");
                            player.SendClientMessage(Colors.SUCCESS, $"*{{ffffff}} Для выдачи админки используйте /testadm");
                            player.LoadInfo();
                        }
                        else
